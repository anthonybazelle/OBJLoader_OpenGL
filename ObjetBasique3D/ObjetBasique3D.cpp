...
#include "../config/stb/stb_image.h"
...
bool Initialize()
{
	...
	camera = new CCamera(camPos, camTarget, camForward);
...
void GestionClavier(unsigned char key, int x, int y)
{
	...
	{
		case 'Z':
			camera->Deplacer("FORWARD", x, y);
			printf("Avancer");
			break;
		case 'Q':
			camera->Deplacer("LEFT", x, y);
			printf("A gauche");
			break;
		case 'D':
			camera->Deplacer("RIGHT", x, y);
			printf("A droite");
			break;
		case 'S':
			camera->Deplacer("BACK", x, y);
			printf("Reculer");
			break;
	}
}
...
void animate()
{
	...
	Esgi::Mat4 worldMatrix = translationMatrix * rotationMatrix;
	//Esgi::Mat4 worldMatrix = rotationMatrix * translationMatrix;
	...
	auto modelview_location = glGetUniformLocation(program, "u_ModelViewMatrix");
	glUniformMatrix4fv(modelview_location, 1, GL_FALSE, modelview.m);
...
int main(int argc, const char* argv[])
{
	...
	bool res = pObjLoader->LoadOBJ("../data/Monkey.obj", vertices, uvs, normals);
